  ...
  def export_to_xlsx
    ...
    Axlsx::Package.new do |p|
      ...
      p.workbook.add_worksheet(:name => "Results") do |sheet|
        ...
        sheet.add_row ["pos","participant","team","fasted lap time","last lap time","laps","avg lap time"]

        race_session_adapter.listing.each_with_index do |entry,index|
          sheet.add_row [index + 1,entry['pilot'].name, entry['pilot'].team,formated_lap_time(entry['fastest_lap']['lap_time']),formated_lap_time(entry['last_lap']['lap_time']),entry['lap_count'],formated_lap_time(entry['avg_lap_time'])]
        end
      end

      p.workbook.add_worksheet(:name => "All laps") do |allLaps|
        allLaps.add_row ["participant", "team", "lap", "time", "created at"]
        ...
        race_session_adapter.pilot_ids.each do |pilot_id|
          ...
          race_session.pilot_race_laps.where(pilot_id: pilot_id).order("lap_num ASC").each do |lap|
            ...
            allLaps.add_row [pilot.name, pilot.team, lap.lap_num, formated_lap_time(lap.lap_time), lap.created_at.to_s ]
          end
        end
      end
